...
export function JsonViewer() {
  ...
  const root = tree[EDITOR_CONSTANTS.ROOT_NODE_ID];
          ...
          return (
            ...
            >
              <div className="flex items-center justify-start space-x-2 px-0">
                ...
                <p className="ml-2 text-xs">
                  {typeof node} {Object.keys(node.props).length} entries
                </p>
              </div>

              <CollapsibleContent className="ml-4 space-y-0 border-l-2 pl-8">
                <RecursiveProps elementProps={node.props} parentKey={node.id} />
              </CollapsibleContent>
            </Collapsible>
          );
        })}
