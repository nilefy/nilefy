...
CREATE TABLE IF NOT EXISTS "workspace_app_queries" (
	...
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp,
	CONSTRAINT "workspace_app_queries_query_name_app_id_unique" UNIQUE("query_name","app_id")
);
--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "users" (
	...
	"conformation_token" varchar(256) NOT NULL,
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "workspaces" ADD CONSTRAINT "workspaces_deleted_by_id_users_id_fk" FOREIGN KEY ("deleted_by_id") REFERENCES "users"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
